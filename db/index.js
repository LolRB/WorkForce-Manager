...
class Database {
  async runQuery(queryText, parameters = []) {
    ...
    try {
      return await client.query(queryText, parameters);
    } finally {
      client.release();
    }
  }
  ...
  getAllEmployees() {
    const query = `
      ...
      FROM employees e
      LEFT JOIN roles r ON e.role_id = r.roles_id 
  ...
  addEmployee({ first_name, last_name, role_id, manager_ref }) {
    
    const query = `
      INSERT INTO employees (first_name, last_name, role_id, manager_ref) 
      VALUES ($1, $2, $3, $4);
    `;
    return this.runQuery(query, [
      first_name,
  ...
  modifyEmployeeRole(employeeId, roleId) {
    const query =
      "UPDATE employees SET role_id = $1 WHERE employees_id = $2;";
    return this.runQuery(query, [roleId, employeeId]);
  }

  modifyEmployeeManager(employeeId, managerId) {
    const query =
